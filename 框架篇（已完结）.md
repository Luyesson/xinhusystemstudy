...
即可以看作是**文件和文件的连接（把另一文件粘贴到引用处）**。因此可以在View.php（视图代码）中看到直接使用`$d` `$m`  `$a`变量。这三个变量贯穿学习始终，简记成：
...
2. 判断是否登录（`initProject`）、数据库加载（`webmainAction.php`）
...
3. 进入login页面输入账号密码（session不存在）登录<mark>或</mark>直接进入对应路由（session存在）
...
1. 图标快捷进入(/home/desktop/items_kjrko.php)

2. 块状内容显示除了(文件夹内kjrko都属于)，这部分放在`首页项管理`下

即1对应图（当然还需要有相应的权限）![](img/2026-03-06-10-36-28-image.png)

2对应

![](img/2026-03-06-10-38-50-image.png)

#### 小结

至此，整个页面启动加载流程步骤就已经讲解完毕；

以下是启动顺序的流程图：

![](img/2026-03-06-14-05-07-image.png)

<mark>框架篇初版结束（3月6日）</mark>
